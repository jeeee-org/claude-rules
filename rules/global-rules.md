...
| ファイル | 役割 | いつ読む | いつ書く |
|---|---|---|---|
| `CLAUDE.md` | PJ固有の指示 | 開始時に必ず | 運用ルール変更時 |
...
- **CLAUDE.mdにセッション履歴を追記しない**（毎回全文ロードされる）。
- 決定（ADR）は独立した節を持たず、決定は日付付き1行で`REQUIREMENTS.md`の方針へ、理由は`NOTES.md`へ。
- **常時ロードされるファイルの上限**（PJのCLAUDE.mdで上書き可）: グローバルCLAUDE.md **14,336B** / PJ CLAUDE.md **6,144B** / PROGRESS.md **60行かつ12,288B**。判定はPJルートで`~/.claude/tools/check-limits.sh`。超過はその場で指摘し、意味を落とさずバイトを削る（畳める記録はcheckpointへ）。

## 3. 進行ルール

### セッション開始時（必須）
1. `CLAUDE.md`（グローバル＋PJ）を読む
...
- **スキル化の自問**：**次にいつ・何に使うかを1文で言える時だけ**1行で提案する（本体は承認後）。同型作業の繰り返しがcheckpoints / `NOTES.md`にあれば作成を推奨に格上げ。実行部分はテスト付きCLI、判断込みは`.claude/skills/<name>/SKILL.md`。
...
**カードに収まらない依頼の中身**（背景・要求の箇条書き・調べたこと）は、**カードと同じ見出し語で1節**を立て、カードからは参照だけにする。**`叶っている状態`はカードの1か所だけ**に置き、節へ複写しない。**節の呼び名と並べ方はPJの`CLAUDE.md`で1つに決める**——依頼が次々来る窓口型は「追加要求: <題>」を横に並べ、叶える状態を数え上げられる案件は「要件」の表を上から順に叶える。**1節50行を目安**とし、超えた分はcheckpointか専用ファイルへ逃がす。着手前のものは1か所にまとめる。
...
リモート・ブランチ戦略・worktree配置先はPJの`CLAUDE.md`に従う。**§5.1・§5.2は全PJ必須**。

- **コミットは1作業ごと**（1ドキュメント・1機能・1設定変更など）。まとまりが完了するたび指示を待たずにcommitする。無関係な変更を混ぜない。作業が無ければ不要。
- **pushは既定で自動**。リモートがあればcommitに続けて**事前承認なし**で`git push`（機微はprivate/publicで管理する前提）。PJの`CLAUDE.md`に「pushはユーザー指示時のみ」とあれば従う。push先は§5.1のworktreeブランチ（main/develop直pushは例外宣言のあるPJだけ）。
...
- worktreeは**PJグループごとに集約**、ディレクトリ名はブランチ名の**slashをhyphenに置換**。配置パス・グループ名はPJの`CLAUDE.md`に書く。
- 低リスクの個人リポでPJの`CLAUDE.md`が「main直接編集・直push」を明示していれば免除。
