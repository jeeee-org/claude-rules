...
| `REQUIREMENTS.md` | 決めたこと（要求・方針・決定・作業・予定）・スコープ・未決事項。**生きている分だけ** | 追加・変更の議論前 | 決まった／変わった／片付いた時 |
...
| `checkpoints/YYYY-MM-DD-作業内容.md` | 詳細作業ログ（追記専用） | 経緯を調べる時だけ | 完了時・セッション終了時 |

> 調査成果物を`notes/`に置くPJでも、メタな学びは`NOTES.md`（PJのCLAUDE.mdで定義）。

## 2. checkpoint方式

- checkpointはリポ直下の`checkpoints/YYYY-MM-DD-作業内容.md`（作業内容は日本語の短い語）。**作業のまとまりごとに1ファイル**、同日でも別作業なら分け、続きは同ファイルに追記。
