...
2. 名前を埋める。形は`作業名-中身`（どちらも日本語の短い語）。各ファイルの見出しを読み、**その日の主な作業**を書く（例：`2026-08-05<TAB>認証の不具合対応-トークン失効の切り分け`）。
   ...
   - **日付で始まる別の形の名前**（`2026-07-16-llm-judge.md`・`2026-07-16_日本語.md`）は、1列目が日付でなく**旧ファイル名**（拡張子なし）の行になる。移す時は`plan`が並べ、候補に日付の後ろを入れる（英語名なら日本語の作業名に直す）。`checkpoints/`の中で改名だけする時は改名済みとみなすので、改名したい分だけ対応表に「旧ファイル名<TAB>名前」の行を足す。
3. `apply --names <scratchpad>/names.tsv` → 最後に`check`が走る。問題があれば直してから進む。
   - コードの中の例示のリンク（`` `[text](url)` ``）と、元々行き先の無いリンクは張り替えない。symlinkのファイル（別のPJの`NOTES.md`を指すものなど）は書き換えず、リンク切れは実体の位置で見る。
