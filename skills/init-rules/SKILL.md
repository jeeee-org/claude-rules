...
- **Git**：`main` 直 push か feature ブランチか／**低リスクの個人リポで「1作業ごとに push まで自動」をオプトインするか**（グローバル §5。明示が無ければ push はユーザー指示時のみ）。
...
- push 方針: <「低リスク個人リポ。1作業ごとに commit + push を事前承認（自動push可）」 or 「push はユーザー指示時のみ」>
...
### 4. 確認とコミット
- 生成したファイル一覧をユーザーに見せる。
- Git ルール（グローバル §5）に従ってコミット（雛形＋ `.gitignore` 等があれば一緒に）。**push はオプトイン宣言がある時だけ**。無ければコミットのみでユーザーに push 可否を確認。
- §2.5 で新規作成したリポは、初回 push（`git push -u origin main`）で origin/main を確立する。これも push 方針に従う（オプトイン宣言があれば自動、無ければ確認）。
