...
3. 不足している次のファイルだけを作る。
   ...
   - `REQUIREMENTS.md`: 目的・スコープ、進行中の作業カード（1作業1枚。何をする／叶っている状態／現在地／待ち／作業ログ）、やること/バックログ、要求（カードに収まらない中身。`AGENTS.md`で宣言した片方の形だけを使い、両方は置かない。「叶っている状態」はカードにあるので複写しない。1節50行が目安）、決めた要求・方針・決定（日付付き1行。理由は`NOTES.md`）、未決事項。生きている分だけを残し、片付いたら消す（経緯はcheckpointに残る）。
