...
### 導入の案内と個人方針の選択（2026-09-26起票）
- **何をする** = ルールを入れる時にAIがオプションを案内しながら入れられるようにする。個人の運用はPJごとに入れる時に選ばせる。コミットの書き方・memoryを使わない・AI署名を付けない・外に出す文面でMarkdownを使わない・応答の書き方は選択にせず全てで必須
- **叶っている状態** = AIが読む導入の手順（スキル）があり、PJ書き込みの道具が個人の運用を選択肢として受け、選んだ内容を覚えて更新でも保つ。テストが通る
- **現在地**（2026-09-26）= 着手
- **作業ログ** = `checkpoints/*-導入の案内と個人方針の選択-*`（0件）
