...
### PJ外の張り替えの取りこぼし（2026-09-18起票）
- **何をする** = `migrate-checkpoints.py apply`がPJの外の参照も張り替えることを、手順と出力の両方で見えるようにする。
- **叶っている状態** = applyの最後にPJの外で変わったファイルが一覧で出る。`migrate-rules`手順2にリポルートで`git status`を見る1行がある。
- **現在地**（2026-09-18）= 起票。
- **作業ログ** = `checkpoints/*-PJ外の張り替え-*`（0件）
