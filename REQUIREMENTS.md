...
### ループの実務の改善（2026-09-25起票）
- **何をする** = 別PCの実務1日の改善メモ（`IMPROVEMENTS.md` 2026-09-25「ループのひな型を実務で1日回した結果」）の残り1・4〜8・12を順番に進める
- **叶っている状態** = 7件がひな型・導入の道具に入り、テストが通り、改善メモに反映の印が付いている
- **現在地**（2026-09-25）= 設計を決めて1（項目ごとの工程の展開）から着手
- **作業ログ** = `checkpoints/*-ループの実務の改善-*`（1件）
...
- ループのひな型を実務で1日回した結果の改善案の残り（`IMPROVEMENTS.md` 2026-09-25「ループのひな型を実務で1日回した結果」の1・4〜8・12）。次は判断役を育てる本体の4〜6（人の答えを選択肢と推奨ごと残す・人待ちの一覧とまとめての裁定）。設計を詰めてから着手。
