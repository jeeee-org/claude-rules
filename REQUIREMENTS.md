...
### ループのひな型の改善（2026-09-25起票）
- **何をする** = 別PCから届いた改善案（`IMPROVEMENTS.md`の2026-09-25「ループのひな型：1工程が止まるとループ全体が異常停止になる、ほか」）を反映する
- **叶っている状態** = 気づき①②③⑤が再現テスト付きで直り、④は判断を仰いで方針が決まっている
- **現在地**（2026-09-25）= ①②③⑤は再現テスト付きで修正済み（159件合格）。残りは④
- **待ち** = ④（範囲の検査を決定論へ移すか）のユーザー判断。未決事項に記載
- **作業ログ** = `checkpoints/*-ループのひな型の改善-*`（1件）
...
## 未決事項
- [ ] ループのひな型の開発用で、「実装が設計の範囲に収まるか」を判断役の問いから決定論ゲートへ移すか（`IMPROVEMENTS.md` 2026-09-25の気づき④）。移すなら、設計の工程に触ってよいファイルの一覧を機械で読める形で書かせ、実行開始時のコミットを状態に残し、実装のゲートで差分と突き合わせる。ひな型の約束ごとが増える。
