...
### 境目の空白の道具化（2026-09-19起票）
- **何をする** = 境目の半角スペースを落とす台本を、`tools/`へテスト付きCLIとして置く
- **叶っている状態** = 他PJへ§9を当てる時に`tools/`のコマンドで済み、保護（行頭マーカー・日付・章番号・コードフェンス・インラインコードの中身）と判断が要る候補の切り分けがテストで守られている
- **現在地**（2026-09-19）= voice-inputへの適用で使った台本がある。道具化とテスト、READMEと移行スキルからの参照はこれから
- **作業ログ** = `checkpoints/*-境目の空白の道具化-*`（1件）
