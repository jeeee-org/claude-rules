...
# ユーザープロンプトを haiku がヘッドレスで T0/T1/T2a に分類し、
# T0 以外のときだけ判定結果をコンテキストに注入する（quorum トリアージの発動漏れ対策）。
...
ctx="自動トリアージ（haiku 分類フック）: ${line} 相当と判定。グローバル CLAUDE.md のトリアージ規則に従い、応答冒頭で宣言し、該当なら /quorum・Fable 単発を1行提案すること。最終判断はメインセッション（この判定は参考情報。ユーザーの明示指定が常に最優先）。"
