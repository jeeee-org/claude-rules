...
## 現在のフェーズ
運用中。2026-09-12に記録ルール（checkpointの粒度・要件と進捗の書き分け・片付け方・ADR節の廃止）を改めて全体を圧縮し、2026-09-15に常時トリアージ規則を廃止（quorumは明示呼び出しだけ）、`migrate-rules`をモノレポで壊れない形にした。2026-09-17に移行コマンドへ改名だけの入口とリンク切れの検査を足し、公開前の点検として業務・個人の具体値を抽象化したうえで履歴を1コミットへ畳みリポを作り直した。2026-09-18に進行中の作業カードを導入し、記録の抜けを機械で捕まえる関門を三段（commitの入口・後追い・pushの関門）で入れた。2026-09-19にカードへ収まらない要求の置き場を決め（形は2つ・PJのCLAUDE.mdで宣言）、Codex側へも広げ、雛形の節名を`決定・方針`へ改め、§9の半角スペースをリポ全体（12ファイル）から落とした。グローバルCLAUDE.mdは12,226B（残り2,110B）。
